...
/* =========================================
   ALPACA LOADER INTERACTION
   ========================================= */
$(document).ready(function () {
  // Only run if the loader exists
  if ($("#site-loader").length) {
    function handleLoaderResponse(message, isCorrect) {
      var $interactionGroup = $("#loader-interaction-group");
      var $resultMessage = $("#loader-result-message");
      var $siteLoader = $("#site-loader");

      // 1. Fade out the buttons/question
      $interactionGroup.fadeOut(400, function () {
        // 2. Show the result message
        $resultMessage.text(message).fadeIn(400);

        // 3. Wait 1.8s then reveal the site
        setTimeout(function () {
          $siteLoader.fadeOut(800, function () {
    ...
    $("#btn-yes").on("click", function () {
      handleLoaderResponse("You know what you hold", true);
    });
...
$(document).ready(function () {
  ...
  $caBtn.on("mouseenter", function () {
    // Only change text if not currently in "Copied!" state and on desktop
    if (!isCopied && window.innerWidth > 991) {
      $(this).text("Copy CA");
    }
  });

  $caBtn.on("mouseleave", function () {
    // Revert to CA if not in "Copied!" state
    if (!isCopied) {
      $(this).text("CA");
    }
  });

  // 2. Click Logic (Copy + Feedback)
  $caBtn.on("click", function () {
    ...
    navigator.clipboard.writeText(contractAddress).then(
      function () {
        ...
        isCopied = true;
        $caBtn.text("Copied!");
        $caBtn.css("border-color", "#ffdd42").css("color", "#ffdd42");

        // Revert after 2 seconds
        setTimeout(function () {
          isCopied = false;
          $caBtn.text("CA");
          // Reset styles to default (CSS hover will take over if mouse is still there)
          $caBtn.attr("style", "");
        }, 2000);
      },
      function (err) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (caButton) {
    ...
    caButton.addEventListener("click", async (e) => {
      e.preventDefault();

      // 1. Get the address from the HTML attribute
      const addressToCopy = caButton.getAttribute("data-copy-value");

      // 2. Clear any existing timer to prevent glitches if clicked fast
      ...
      try {
        ...
        // 4. Add the class that triggers the CSS "Copied" state
        caButton.classList.add("is-copied");

        // Optional: Log to console for debugging
        console.log("Address copied:", addressToCopy);

        // 5. Remove the class after 2 seconds to revert text
        copyTimeout = setTimeout(() => {
          caButton.classList.remove("is-copied");
        }, 2000);
      } catch (err) {
